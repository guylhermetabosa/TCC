...
A documentação de software é uma das atividades mais importantes da atividade de manutenção, pois auxilia uma comunicação com uma equipe externa de manutenção \cite{sergio2005}. Sua importância torna-se mais evidenciada quando Wong (1995 apud SOUZA et al., 2005) afirma que que mais 50\% do esforço de evolução do software é aplicado no entendimento do programa e sua estrutura. \citeonline{sergio2005} identificaram os artefatos mais utilizadas por mantenedores, segundo eles de vinte e quatro artefatos disponíveis, quatorze foram utilizados, o que retrata bem a importância dos artefatos durante a manutenção.


\section{Manutenibilidade de Software}\label{manutenibilidade}
...
Dentre estas divisões de qualidade o foco deste trabalho é a manutenibilidade. A manutenibilidade divide-se ainda em mais cinco subcaracterísticas, são elas:
\begin{itemize}
...
\label{fig:ISO9126}
\legend {\fontsize{10}{12}\selectfont {Fonte: \citeonline{rdribeiro2012}}.}
\end{figure}
...
\subsubsection{Métrica de Halstead}
Halstead (1977 apud FERREIRA, 2012) foi a primeira teoria a associar leis quantitativas ao desenvolvimento de software, por meio de um conjunto de medidas que podem ser retiradas do código após sua geração. Essas medidas foram definidas como operandos, que são valores  que operam alguma instrução, e operadores que são instruções que modificam operandos. Tem-se:
\begin{itemize}
...
\label{fig:Halstead}
\legend {\fontsize{10}{12}\selectfont {Fonte: \citeonline{isaias2012}}.}
\end{figure}

Onde segundo \citeonline{isaias2012} define-se como:
\begin{itemize}
...
Esta complexida foi criada em 1976  por Thomas McCabe, e tem como objetivo medir o número de caminhos independentes de um módulo \cite{isaias2012}.
...
O Índice de manutenibilidade foi desenvolvido em 1994 por Oman. Esta métrica tem o objetivo de mensurar a manutenibilidade do software auxiliando no processo de manutenção, além de utilizar das medidas de Halstead e a Complexidade Ciclomática de McCabe \cite{isaias2012}.
\begin{figure}[h]
\centering
...
\legend {\fontsize{10}{12}\selectfont {Fonte: \citeonline{isaias2012}}.}
\end{figure}
Segundo \citeonline{isaias2012} as medidas são descritas como:
\begin{itemize}
...
\includegraphics[width=0.4\linewidth]{./images/build}
\label{fig:build}
...
\begin{OnehalfSpace}
A integração contínua tem como objetivo identificar erros o mais rápido possível, ela permite que alterações efetuadas e integradas aos repositórios dos sistemas de controle de versão (SCV) sejam posteriormente verificadas e caso erros ocorram, estes serão notificados imediatamente ao autor da alteração.
